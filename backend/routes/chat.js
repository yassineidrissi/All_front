...
// chatRouter.post('/api/auth/best_prompt', async (req, res) => {
//     try {
//         const { prompt } = req.body;
...
// Modify the /api/auth/best_prompt endpoint in routes/chat.js or auth.js
chatRouter.post('/auth/best_prompt', async (req, res) => {
    try {
        const { prompt } = req.body;
        ...
        // But make sure to return the exact format expected by frontend:
        
        res.json({
            original: {
                prompt: prompt,
                response: "This is the original response for: " + prompt,
                score: 0.75
            },
            optimized: {
                prompt: "Improved: " + prompt,
                response: "This is the optimized response",
                score: 0.92
            }
        });
        
    } catch (error) {
        console.error('Error in best_prompt:', error);
